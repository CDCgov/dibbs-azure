...
terraform {
  backend "azurerm" {
    resource_group_name  = "skylight-dibbs-global" // TODO: Change this to match the resource group that contains your storage account for Terraform state storage.
    storage_account_name = "dibbsstatestorage"     // TODO: Change this to match the storage account that contains/will contain your Terraform state files.
    container_name       = "dibbs-tfstate"         // We recommend leaving this alone, to keep DIBBs state files separate from the rest of your resources.
